...
def run_paper_session(
    ...
    *,
    ...
    summary["message"] = "Paper session completed using the configured execution backend."
    return summary
...
def _execute_phase7_run(
    ...
    *,
    ...
    for index, row in ordered_frame.iterrows():
        ...
        portfolio_before = position_manager.update_market_prices({symbol: mark_price}) if symbol and mark_price is not None else position_manager.snapshot()
        ...
        portfolio_after = position_manager.update_market_prices({symbol: mark_price}) if symbol and mark_price is not None else position_manager.snapshot()
        record = {
